...
{
    public partial class UserControlPrzekaski : UserControl
    {
        public UserControlPrzekaski()
        {
            InitializeComponent();
        }
        ...
        private void btnDodajDoKoszyka_Click(object sender, EventArgs e)
        {
            ...
            {
                ListViewItem prod = new ListViewItem(listViewPrzekaski.SelectedItems[0].Text);
                prod.SubItems.Add(x.ToString());
                int cena = int.Parse(listViewPrzekaski.SelectedItems[0].SubItems[1].Text);
